...
export function FailingItems({ items }: { items: ItemRow[] }) {
  ...
  if (failing.length === 0) {
    return <p className="text-sm text-neutral-500">No failing items.</p>
  }
  return (
  ...
  return (
    <li>
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="flex w-full items-center gap-3 px-3 py-2.5 text-left hover:bg-white/5 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400"
        aria-expanded={open}
      >
        <span
          aria-hidden
        ...
        <span className="flex-1 truncate text-sm text-neutral-300">{item.input}</span>
        <span className="bg-rose-500/15 px-2 py-0.5 text-[11px] font-medium text-rose-400">
          failed
        </span>
      </button>
      {open && (
        <div className="space-y-2 bg-black/20 px-4 py-3 text-sm">
...
function Field({
  ...
  const color =
    tone === 'bad' ? 'text-rose-400' : tone === 'ok' ? 'text-emerald-400' : 'text-neutral-300'
  return (
